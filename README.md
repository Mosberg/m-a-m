...
**Mana And Magic** is a flexible, data-driven magic system for Minecraft 1.21.11 that lets you cast spells from multiple schools, harness mana across three pools, and progress through tiered spellbooks and staffs. Built with strict architectural separation using Fabric Loom's split source sets for clean client/server boundaries.

### ✨ Core Features

- **4 Spell Schools** — Air, Earth, Fire, Water with school-specific projectile visuals and impact effects
- **Tiered Spell Progression** — Novice (Tier 1) through Master (Tier 4), with 8 core spells (tier 1 & 2) ready to extend
- **Gemstone Binding** — Ruby, Sapphire, Moonstone, Peridot each unlock their corresponding spell school
- **3-Pool Mana System** — Personal, Aura, and Reserve pools with customizable regeneration and decay
- **Dynamic Projectiles** — School-themed particle trails scaled by spell tier; thematic impact bursts on collision
- **GUI Spell Selection** — Press `R` with a spellbook to browse and select spells; right-click to cast
- **Data-Driven Content** — Extend spells, items, and mechanics via JSON without touching code
- **Split Architecture** — Strict client/server separation with Fabric's split source sets; easy to maintain and extend
...
- Minecraft Launcher with Fabric Loader 0.18.4+
...
# Generate mod data (assets, recipes, etc.)
./gradlew runDatagen
```

### Installation

1. Download the latest release JAR from [Releases](https://github.com/mosberg/mam/releases)
...
1. **Find or craft a Spellbook** (Novice tier to start)
2. **Obtain a Gemstone** (Ruby, Sapphire, Moonstone, or Peridot)
3. **Bind the gemstone to your spellbook** (mechanics vary; see item tooltips)
4. **Hold the spellbook, press `R`** to open spell selection
5. **Right-click to cast** — Watch your mana deplete and see school-themed particle bursts!

### Spell Schools & Gemstones

| School    | Gemstone  | Focus               | Example Spell                               |
| --------- | --------- | ------------------- | ------------------------------------------- |
| **Fire**  | Ruby      | Damage, DoT         | Fire Strike (Tier 1), Fire Bolt (Tier 2)    |
| **Water** | Sapphire  | Healing, Slow       | Water Strike (Tier 1), Water Jet (Tier 2)   |
| **Air**   | Moonstone | Mobility, Knockback | Air Strike (Tier 1), Air Gust (Tier 2)      |
| **Earth** | Peridot   | Defense, Stun       | Earth Strike (Tier 1), Earth Shard (Tier 2) |

### Progression

- **Novice Tier** — Basic spells, lower mana costs, perfect for learning
- **Apprentice Tier** — Intermediate spells with stronger effects
- **Adept Tier** — Advanced spells with area effects and status conditions
- **Master Tier** — Powerful spells with high damage and extended range
...
mam/
├── src/
│   ├── client/          # Client-only code (rendering, keybinds, GUI)
│   │   ├── java/
│   │   │   └── dk.mosberg.client/
│   │   │       ├── MAMClient.java
│   │   │       ├── render/
│   │   │       ├── screen/
│   │   │       └── keybind/
│   │   └── resources/   # Client assets (textures, sounds, etc.)
│   └── main/            # Server & common code (items, spells, entities)
│       ├── java/
│       │   └── dk.mosberg/
│       │       ├── MAM.java
│       │       ├── entity/
...
│       │       └── network/
│       └── resources/
│           ├── assets/mam/    # Item models, textures, lang files
│           ├── data/mam/      # Spell JSON definitions
│           └── fabric.mod.json
├── gradle.properties    # Version & metadata (auto-syncs to fabric.mod.json)
└── build.gradle         # Gradle build script
```

### Key Packages

- **`dk.mosberg.MAM`** — Main mod initializer; item/entity/network registration
- **`dk.mosberg.entity`** — Custom entities (SpellProjectileEntity)
- **`dk.mosberg.spell`** — Spell data model and casting logic
- **`dk.mosberg.client`** — Client-only features (rendering, keybinds, screens)
- **`dk.mosberg.network`** — Networking payloads and handlers

### Code Patterns

#### Strict Split Source Sets

- `src/main/` → Server + common code (runs on both sides)
- `src/client/` → Client-only code (never imported by server)
- **Rule:** Server code cannot import `dk.mosberg.client.*`

#### Registry Pattern

All items, blocks, and entities register in `MAM.onInitialize()`:
...
Define spells in JSON under `data/mam/spells/{school}/`:
...
```json
{
  ...
  "range": 40.0,
  "projectileSpeed": 1.5,
  "tags": ["strike", "projectile", "tier1", "fire"]
}
```
...
1. **Create spell JSON** in `src/main/resources/data/mam/spells/{school}/`
2. **Define stats:** `manaCost`, `cooldown`, `damage`, `range`, `projectileSpeed`, etc.
3. **Add translations** in `assets/mam/lang/en_us.json`
4. **Add particle VFX** config if using custom effects
5. **Run `./gradlew runClient`** — Spell loads automatically

### Adding a New Spell School

1. Add school enum variant to `SpellSchool`
2. Create gemstone item and assets
3. Create corresponding projectile item with model/texture
4. Add spells under new school directory
5. Update UI/HUD to show new school option

### Adding a New Tier

1. Create spellbook/staff items for the tier
2. Add tier-specific spells
3. Adjust `manaCost` and progression thresholds
4. Test scaling with existing spells
...
All mod metadata is defined in `gradle.properties` and auto-syncs to `fabric.mod.json` during build:
...
mod_description=Data-driven magic system for Minecraft
...
**Important:** Do not edit `fabric.mod.json` directly—it's auto-generated.

### Server Config

Create `mam-server.properties` in the server config folder to customize:
...
```bash
./gradlew runDatagen
# Output: src/main/generated/resources/
```
...
| ----------------- | --------------- | --------------------------------- |
| **Minecraft**     | 1.21.11         | Game engine                       |
| **Fabric Loader** | 0.18.4          | Mod loader                        |
| **Yarn Mappings** | 1.21.11+build.3 | Readable class/method names       |
| **Fabric API**    | 0.141.1+1.21.11 | Hooks, events, networking         |
| **Fabric Loom**   | 1.14.10         | Build tool with split source sets |
...
2. **Keep code clean** — Follow Yarn naming conventions and existing patterns
3. **Test locally** — Run `./gradlew runClient` and verify your changes
4. **Document changes** — Update README and code comments as needed
5. **Submit a PR** with a clear description of your changes

### Code Style

- Use Yarn mappings (readable names)
- Keep client and server code separated
- Use data-driven JSON for content where possible
...
- Check `data/mam/spells/` JSON for syntax errors
- Look for errors in server log during startup
- Verify spell JSON matches the schema (see examples above)

### Client Crashes After Installing

- Ensure Fabric Loader 0.18.4+ is installed
- Check mod compatibility with other mods
- Report issue on [GitHub Issues](https://github.com/mosberg/mam/issues)

---

## Performance Notes

- **Particle scaling:** Trail particles scale with spell tier for visual feedback without excessive load
- **Mana regen:** Server-side calculations; clients sync via network payloads
- **Projectiles:** Physics-based, no pathfinding; safe for multiplayer
- **Gradle:** Configuration cache and parallel builds (~20% speedup on subsequent runs)

---

## Roadmap

- [ ] Tier 3 & 4 spells for all schools
- [ ] Spell fusion/combining for unique effects
- [ ] Mana pools UI improvements
- [ ] Server-side configuration GUI
- [ ] Custom particle effects editor
- [ ] Spell cooldown UI overlay
- [ ] Multiplayer testing & balancing
...
- **Yarn Mappings** — FabricMC for Minecraft 1.21.11 mappings
- **Fabric API** — FabricMC for modding hooks and events
- **Loom** — FabricMC for seamless build integration
- **Community** — Thanks to everyone who tests and provides feedback!

---

## Support

Have questions or found a bug? [Open an issue](https://github.com/mosberg/mam/issues) on GitHub or check the [Discussions](https://github.com/mosberg/mam/discussions) tab.

**Happy spell casting!** ✨🔥💧🌪️⛰️
